...
def read_arguments_v3():
    ...
    parser.add_argument('-r', '--virus_reference', 
            ...
            [Default is SARSCoV2_Wuhan1.fasta]""")
    parser.add_argument('-m', '--margin', 
            help=""" Number of positions to take into account for the average
            post and pre coordinate metrics. [Default is 5].""")
    ...
    parser.add_argument('-n', '--n_processes', 
            help="Number of processes. [Default is 1].")
    parser.add_argument('-s', '--polarity',
            help="""Indicate the polarity of the virus as 1 ('+', positive polarity)
             or 0 ('-', negative polarity). [Default is 1, i.e. positive].""")
    ...
    return fq, virus_ref, virus_ref_path, margin, threshold, n_processes, polarity
...
def create_cIDDI_woID(sense, BP, RI):
    """
    Generates cID_DI from sense, BP and RI
    ...
    Return:
        cID_DI  (str)   [out]   Identificador completo = sense_BP_RI
    """
    cDI_ID = str(sense) + "_" + str(BP) + "_" + str(RI)
...
def invert_polarity(df):
    """
    If the polarity of the viral genome is -, the function changes the terminology:
    In DVG_type:
        5cb <--> 3cb
        forward <--> reverse
    In sense:
    ...
    Args:
        df  (pd.DataFrame)  Df with the +sense terminology
    returns:
        df- (pd.DataFrame)  Df with the - sense terminology
    """
    dvg_type = {"Deletion_reverse" : "Deletion_forward",
                "Deletion_forward" : "Deletion_reverse",
    ...
    df_negative = df[:] # unlinked copy

    if "DVG_type" in df_negative.columns:
        df_negative = df_negative.replace({"DVG_type" : dvg_type})
    if "sense" in df_negative.columns:
        df_negative = df_negative.replace({"sense" : sense})
    if "cID_DI" in df_negative.columns:
        df_negative.drop("cID_DI", inplace=True, axis=1)
        if "ID_DI" in df_negative.columns:
            df_negative.drop("ID_DI", inplace=True, axis=1)
        cIDDI_col = df_negative.apply(lambda row: 
                    create_cIDDI_woID(row['sense'], row['BP'], row['RI']),
                    axis=1)
        df_negative.insert(0, 'cID_DI', cIDDI_col)
